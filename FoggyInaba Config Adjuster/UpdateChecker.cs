...
namespace FoggyInaba_Config_Adjuster
{
	/// <summary>
	...
	public class UpdateChecker
	{
		...
		public async Task CheckForUpdatesAsync(string currentVersion)
		{
				...
				if (response.IsSuccessStatusCode)
				{
					...
					if (release != null && IsNewerVersion(release.tag_name, currentVersion))
					{
						var promptWin = new PromptWindow(
							title: FoggyInabaConfig.Localisation.LocalisationResources.Resources.Update_Notify_Title+release.tag_name,
							content: FoggyInabaConfig.Localisation.LocalisationResources.Resources.Update_Notify
						);
						...
						else
						{
							var infoWin = new InfoWindow(
								FoggyInabaConfig.Localisation.LocalisationResources.Resources.Update_Later_Title,
								FoggyInabaConfig.Localisation.LocalisationResources.Resources.Update_Later
							);
							infoWin.ShowDialog();
						}
					}
			...
			catch (Exception ex)
			{
				...
				var infoWin = new InfoWindow("Update Error", FoggyInabaConfig.Localisation.LocalisationResources.Resources.Update_Check_Fail);
				infoWin.ShowDialog();
			}
		}
