    ...
    private void HandleDownloadUrl(string url, DownloadHandler handler, string? githubowner, string? githubreponame, string? regex)
    {
        ...
        else if (handler == DownloadHandler.GithubReloadedDirectDL)
        {
            if(githubowner  != null && githubreponame != null)
            {
                ...
                var infoWin = new InfoWindow("Download Info", FoggyInabaConfig.Localisation.LocalisationResources.Resources.Mod_Download_Start);
                infoWin.ShowDialog();
            }
            else
            {
                var infoWin = new InfoWindow("Download Error", FoggyInabaConfig.Localisation.LocalisationResources.Resources.Mod_Download_Error);
                infoWin.ShowDialog();
            }
        }
    }
...
public static async void GithubR2Direct7z(string owner, string repo, string? includereg)
{
    ...
    catch
    {
        var infoWin = new InfoWindow(
            ...
            FoggyInabaConfig.Localisation.LocalisationResources.Resources.ModInfoRetrievalFail,
            "Installation Failed"
        );
        infoWin.ShowDialog();
    }
